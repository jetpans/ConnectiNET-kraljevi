...
export default function EventDetail(props) {
    ...
    const allComments = comments && comments !== null ? comments.map((comment) => (
    ...
    const fetchData = async () => {
        if(props && props.event) {
            ...
            dc.GetData(API_URL + "/getEvent/" + props.event.id, accessToken).then((resp) => {
  ...
  return (
    <Box sx={{ overflowY: 'scroll', maxHeight: '90vh', width: '70vw' }}> 
       {props && props.event && props.closeDialog && cards && cards.id != null ? (
        <Container maxWidth="lg">

            <Card
                sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: theme.palette.background.default, color: theme.palette.text, width: '60vw' }}
                elevation={24}
            ...
            >
                ...
                >
                    <Box sx={{marginBottom: 0}}>
                        ...
                        >
                            <UserUploadedImage src = {"/"+cards.image_org} style={{ maxWidth: '200px', maxHeight: '200px' }}/> 
                        </div>
                        <div>
                            <UserUploadedEventImage src="/placeholder.jpg" />
                        </div>
                    </Box>
                </CardMedia>
                <CardContent sx={{ flexGrow: 1 }} key={props.event.id}>
                    <Typography gutterBottom variant="h3" sx={{marginBottom: 0}} color={theme.palette.text.main}>
                        {cards.title}
                    </Typography>
                    <Button variant="h5" sx={{marginBottom: 3, color: theme.palette.primary.main, marginLeft: 0, paddingLeft: 0}} onClick={() => {handleCloseWithReroute("/organizer/" + cards.accountId)}}>
                        {cards.organizer}
